...
//! of hand-rolling their own copies; `rdlp-api` imports those two directly
//! from `rdlp_plugin::test_support` since it has no `mod common` of its
//! own). This module is kept only so the many `mod common; use
//! common::{...}` call sites in this crate's own `tests/` binaries don't
//! all need rewriting to the new path.
