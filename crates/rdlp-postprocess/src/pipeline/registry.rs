...
impl TempRegistry {
    ...
    // Sync `std::fs` on purpose, and reached from async callers
    // (`FileTracker::temp_path`, `PartLock::claim` — rdlp#572): the work is
    // one local metadata syscall plus a `flock` on the sidecar, and the lock
    // fd must be created and registered under the registry's `std::sync`
    // mutex, which cannot be held across an `.await` — so `spawn_blocking`
    // is not an option here without restructuring the registry.
