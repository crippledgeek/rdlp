...
export const searchParamsAtom = new Store<SearchParams>(initialState);

/** Set a single field on the search params atom. */
export function setSearchParam<K extends keyof SearchParams>(
    key: K,
    value: SearchParams[K],
): void {
    searchParamsAtom.setState((prev) => ({ ...prev, [key]: value }));
}

/** Reset the search form to initial state. */
export function resetSearchParams(): void {
    searchParamsAtom.setState(() => ({ ...initialState }));
}
