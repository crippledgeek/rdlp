            ...
            Self::Preparing {
                ...
                let (state, part_lock) = match plan.plan() {
                    ...
                    DownloadPlan::Single(_) => {
                        ...
                        // Claim the output path BEFORE resume resolution (#572):
                        ...
                        // The complete/resume/fresh decision is the outcome's
                        // (#561); this only maps it onto a `DownloadState`.
                        let (lock, outcome) = orchestrator
                            .claim_part_and_resolve_resume(&part, format.filesize)
                            .await?;
                        let state = match outcome {
                            super::resume::ResumeOutcome::Complete { .. } => {
                                // Already complete: the bytes are under .rdlp-part;
                                // commit them to the clean name before returning
                                // Complete. `lock` drops (releasing the claim)
