...
// drops it explicitly as the LAST statement, after its assertions run. Same
...
// access (registering the mocks), but dropping a `ServerGuard` recycles the
// underlying `Server` back to mockito's server pool, and that recycle step
// calls `reset()`, which clears every mock the server had registered —
// doing that before the awaited request and the `.expect(N)` mock-drop
// assertions run would break the test, not just release a resource earlier.
// Moving the real last use (this explicit `drop`) to the true end of the
// function closes the gap the lint flags without changing when the server
// actually goes away.
