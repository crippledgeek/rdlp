...
//! - `info` - Single-video extractor (`TNAFlixExtractor`)
//! - `search` - TNAFlix search extractor (`TNAFlixSearchExtractor`)
//! - `tnaflix_search_helpers` - HTML search result parsing for TNAFlix
//! - `search_patterns` - Search URL builders and filter descriptors
//! - `empflix_search` - EMPFlix search extractor (`EMPFlixSearchExtractor`)
//! - `moviefap_search` - MovieFap search extractor (`MovieFapSearchExtractor`)
...
pub use empflix_search::EMPFlixSearchExtractor;
pub use info::TNAFlixExtractor;
pub use moviefap_search::MovieFapSearchExtractor;
pub use search::TNAFlixSearchExtractor;

#[cfg(test)]
use crate::base::common::BaseExtractor;
#[cfg(test)]
use rdlp_core::SearchExtractor;
...
mod async_tests {
    ...
    impl CookieJar for NoOpCookieJar {
        async fn cookies(&self, _url: &str) -> rdlp_core::Result<Vec<String>> {
            Ok(vec![])
        }
        async fn add_cookie(&self, _url: &str, _cookie: &str) -> rdlp_core::Result<()> {
            Ok(())
        }
        async fn load_from_browser(
            &self,
            _browser: rdlp_types::BrowserType,
        ) -> rdlp_core::Result<usize> {
            Ok(0)
        }
        async fn load_from_file(&self, _path: &std::path::Path) -> rdlp_core::Result<usize> {
            Ok(0)
        }
    }
